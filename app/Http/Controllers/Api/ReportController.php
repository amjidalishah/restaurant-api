...
class ReportController extends Controller
{
    ...
    /**
     * Get order type sales breakdown
     */
    public function orderTypeSales(Request $request): JsonResponse
    {
        ...
        $query = Order::select(
            DB::raw("COALESCE(type, 'unknown') as type"),
            DB::raw('COUNT(*) as total_orders'),
            DB::raw('SUM(total) as total_sales'),
            DB::raw('AVG(total) as average_order')
        );

        $this->applyPeriodFilterToOrderQuery($query, $period);

        $orderTypeSales = $query->groupBy('type')
    ...
    public function hourlySales(Request $request): JsonResponse
    {
        ...
        [$hourExpression, $hourSelect, $hourOrder] = $this->getHourExpressions();

        $hourlySales = Order::select(
    ...
    /**
     * Apply common period filter to an order query
     */
    protected function applyPeriodFilterToOrderQuery($query, string $period): void
    {
        switch ($period) {
            case 'today':
                $query->whereDate('created_at', today());
    ...
    /**
     * Get database-agnostic hour expressions
     */
    protected function getHourExpressions(): array
    {
